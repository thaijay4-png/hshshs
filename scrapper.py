...
async def scr_cmd(client, message):
    ...
    if len(args) < 2 or len(args) > 3:
        await message.reply_text("<b>⚠️ Provide channel username and amount to scrap</b>")
        return
    channel_identifier = args[0]
    limit = int(args[1])
    max_lim = ADMIN_LIMIT if message.from_user.id in ADMIN_IDS else DEFAULT_LIMIT
    if limit > max_lim:
        await message.reply_text(f"<b>Error Amount over Max limit is {max_lim} ❌</b>")
        return
    ...
    if unique_messages:
        ...
        with open(file_name, 'rb') as f:
            caption = (
                ...
                f"<b>━━━━━━━━━━━━━━━━</b>\n"
                f"<b>Card-Scrapper By: <a href='https://t.me/binmethodk'>ArcaxBydz</a></b>\n"
            )
